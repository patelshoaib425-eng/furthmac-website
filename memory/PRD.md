...
## Static HTML/CSS Version (built 2026-07)
- Full static port at /app/static-site/ — 27 HTML pages, single stylesheet (assets/styles.css),
  tiny vanilla JS (assets/main.js: mobile menu, sticky header, scroll reveals), Lucide icons via CDN.
- Generated by /app/static-site/generate.py (all content lives there; run `python3 generate.py` after edits).
- Pages: index, about, services index, 15 service pages (services/<slug>.html), industries, careers,
  blog, contact, privacy, terms, cookie-policy, 404, thanks. Plus sitemap.xml, robots.txt, .htaccess.
- Contact form: FormSubmit → action https://formsubmit.co/info@furthmac.com, _next → /thanks.html.
  ONE-TIME ACTIVATION REQUIRED: first submission sends a confirm email to info@furthmac.com — click it.
- Tested via testing agent: 100% pass (all 27 pages, dropdown, mobile 375px, FAQ accordions, links,
  form attributes, sitemap/robots). Instagram icon replaced with inline SVG (lucide UMD lacks it).
- Upload target: Hostinger public_html (user already hosts domain there). React app remains intact.

## Backlog
- P1: Upload /app/static-site to Hostinger public_html; click FormSubmit activation link on first enquiry.
...
- P2 (optional): Add credits / upgrade plan to deploy the React version on Emergent (50 ECU/month, balance 34).
